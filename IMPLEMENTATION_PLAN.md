...
### 6.2 List/Search (`ls`)
- Input: `[-l <limit>] [-wa] [-f <from>] [-c <contains>] [-tl <time_limit>]`.
- `ls` parser v1 grammar:
  - one optional explicit `-l <limit>` for `maxResults`
  - `-wa` keeps only messages with downloadable attachments, excluding `.ics`-only invites
...
- Conversion to Gmail query:
  ...
  - `-wa -l 10` -> Gmail `q="-in:sent has:attachment"` followed by local filtering of `.ics`-only invite messages, `maxResults=10`.
  - `-f geeta -tl 2w -l 10` -> Gmail `q="from:geeta newer_than:14d"`, `maxResults=10`.
  - `-tl "jan 2025" -l 20` -> Gmail `q="after:2024/12/31 before:2025/02/01"`, `maxResults=20`.
- API sequence:
